...
const addNote = (noteName) => {
    ...
    cy.get('.note-wrapper').first().contains(DEFAULT_NOTE_NAME).should('be.visible')
    cy.get('.note-wrapper').first().contains(DEFAULT_NOTE_NAME).click({ force: true })
...
describe("Add New Note", function () {
    ...
    it.skip('adds profile to note and the note is appear in the selected user', () => {
        ...
        cy.get('mat-dialog-container .labels-list').contains('Notes').click({ force: true })
