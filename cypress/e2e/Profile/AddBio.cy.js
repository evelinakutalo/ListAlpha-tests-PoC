...
describe("Add Bio", () => {
  ...
  it(`should: create user, add new bio and delete him`, () => {
    ...
    cy.get(toastBlock).contains("added");

    cy.wait(5000);

    cy.get(".lists-container")
      .find(".user-wrapper", expected)
      .eq(0)
      .should("exist");
    ...
    //add notes
    cy.get(".lists-container").find(".user-wrapper", expected).eq(0).click();
    ...
    cy.get(".lists-container").find(".user-wrapper").eq(0).contains(expected);
